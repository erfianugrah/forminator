		...
		<Dialog open={!!entry} onClose={onClose}>
			<DialogContent className="p-0 max-w-3xl max-h-[85vh] overflow-y-auto">
				...
				<div className="p-6 space-y-6">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
						...
						<div className="space-y-2">
							...
							<div className="text-xs space-y-1">
								{entry.ephemeral_id && (
									<p>
										<span className="font-medium">Ephemeral ID:</span>{' '}
										<span className="font-mono select-all break-all">{entry.ephemeral_id}</span>
									</p>
								)}
								{entry.ip_address && (
									<p>
										<span className="font-medium">IP:</span> <span className="font-mono select-all break-all">{entry.ip_address}</span>
									</p>
								)}
								{entry.ja4 && (
									<p>
										<span className="font-medium">JA4:</span> <span className="font-mono select-all break-all">{entry.ja4}</span>
									</p>
								)}
							</div>
						</div>
					...
					{/* Full Risk Score Breakdown (FraudAssessment) */}
					{(() => {
						if (!entry.risk_score_breakdown) return null;
						try {
							const breakdown = JSON.parse(entry.risk_score_breakdown);
							if (breakdown && typeof breakdown === 'object' && breakdown.components) {
					...
					<div className="space-y-2">
						<p className="text-muted-foreground text-sm font-medium">Block Reason</p>
						<div className="bg-muted/60 rounded-md p-3 text-sm border border-border/50">
							<p className="text-foreground">{entry.block_reason}</p>
						</div>
					</div>

					{metadata && (
						<details className="space-y-2">
							<summary className="text-muted-foreground text-sm font-medium cursor-pointer hover:text-foreground">
								Detection Metadata (raw)
							</summary>
							<pre className="bg-muted/40 rounded-md p-3 text-xs border border-border/50 overflow-x-auto whitespace-pre-wrap font-mono">
