...
export function SubmissionFlow({ currentStep, errorMessage }: SubmissionFlowProps) {
	...
	const getStepStatus = (stepId: FlowStep) => {
		...
		if (stepId === 'turnstile-challenge') {
			...
			if (['turnstile-challenge', 'turnstile-interactive', 'turnstile-success'].includes(currentStep)) {
				return 'active';
			}
			if (currentIndex > 2) {
				return 'complete';
						...
						<div key={step.id} className="flex flex-col items-center relative z-10" style={{ flex: 1 }}>
							...
							<div
								className={cn(
									...
									status === 'complete' && 'bg-primary text-primary-foreground shadow-lg',
									status === 'active' && 'bg-primary text-primary-foreground shadow-lg ring-4 ring-primary/20',
